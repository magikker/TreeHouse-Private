...
void compress(string file1, bool bUbid, bool quiet, vector<string> nexTaxa, bool use) {
  ...
  LabelMap lm = collect_labels_light(file1); //collect labels and number of trees
  ...
  if (WEIGHTED){
    cout << "Automatic check: Trees are weighted!" << endl;
  }
  else{
    cout << "Automatic check: Trees are unweighted!" << endl;
  }
