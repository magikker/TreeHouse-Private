...
static NEWICKNODE *loadnode(FILE *fp, int *error, NEWICKNODE *parent);
static int addchild(NEWICKNODE *parent, NEWICKNODE *child);
static NEWICKNODE *loadleaf(FILE *fp, int *error, NEWICKNODE *parent);
...
NEWICKTREE *floadnewicktree(FILE *fp, int *error)
{
  ...
  if(ch == '(' )
  {
    answer->root = loadnode(fp, &err, NULL);
...
NEWICKTREE *stringloadnewicktree(char *subtree, int *error)
{
  ...
  FILE *fp;
  fp = fmemopen(subtree, strlen(subtree),"r");
...
static int addchild(NEWICKNODE *parent, NEWICKNODE *child)
{
  ...
  parent->Nchildren++;
  //parent->child[parent->Nchildren]->myparent = parent;
  return 0;
}
