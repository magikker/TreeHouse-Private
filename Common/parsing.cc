...
#define add_of(c,a,b) ({ \
  ...
  __typeof__(b) __b=b; \
  (__b)<1 ? \
    ((__MIN(__typeof__(c))-(__b)<=(__a)) ? assign(c,__a+__b):1) : \
    ((__MAX(__typeof__(c))-(__b)>=(__a)) ? assign(c,__a+__b):1); \
})
...
LabelMap collect_labels(string file1) {
  ...
  FILE *fp;
  LabelMap lm;
  fp = fopen(file1.c_str(), "r"); //all labels MUST be populated by the same file -- thus the need for a common seed file
  ...
  for (unsigned int x = 0; x < NUM_TREES; x++){ 
    ...
    if (x == 0)
      initial_ntaxa = lm.size();
    if ( (lm.size() > initial_ntaxa) && (HETERO!= true)){
