...
set<unsigned int> clade_size_search(vector<int> required, int size)
{
	...
	for(int i = 0; i < B.goodBipartitions.size(); i++){ //for each bipartition whose trees we want
		...
		for(int j = 0; j < biparttable.searchtable[B.goodBipartitions[i]].size(); j++){ //for each tree in the searchtable
			if(B.heteroMode.at(i)){ //this means the clade we found contained all 0s and the data set is hetero
				...
				int tree = biparttable.searchtable[B.goodBipartitions[i]][j];
				bSize = bSize + num_taxa_in_tree(tree) - ::NUM_TAXA; //since the clade was all 0s, n of those 0s don't actually exist in the tree, NUM_TAXA - num_taxa_in_tree(tree)
				//cout << "Tree " << tree << ", adjusted bSize is " << bSize << endl;
				if(bSize == size && are_taxa_in_tree(tree, required)){
					retSet.insert(biparttable.searchtable[B.goodBipartitions[i]][j]);
					}
				}
			else{			
				retSet.insert(biparttable.searchtable[B.goodBipartitions[i]][j]);
...
set<unsigned int> smallest_clade(vector<int> required){
	...
	if(!::HETERO) {
		...
		for(int i = 0; i < B.goodBipartitions.size(); i++){ //for each good bipartition
			for(int j = 0; j < biparttable.searchtable[B.goodBipartitions[i]].size(); j++){ //for each tree with that bipartition
				int tree = biparttable.searchtable[B.goodBipartitions[i]][j];				
				...
				if(B.allZeros.at(i)){ //if the clade is all 0s, we have to make sure all the required biparts are in it and adjust the size
					bSize = bSize + num_taxa_in_tree(tree) - ::NUM_TAXA;
					//also check that all the required taxa exist in this tree
					if(!are_taxa_in_tree(tree, required)){cout << "Taxa not in tree " << tree << endl; bSize = ::NUM_TAXA + 1;} //make it impossible for bSize to be smallest 
					}
